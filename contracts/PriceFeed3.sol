...
contract PriceFeed3 is IPriceFeed3, OwnableUpgradeable, UUPSUpgradeable, Addresses {
	...
	function _fetchPythOracleResponse(
		address pythAddress,
		bytes32 id
	) public view returns (uint256 price, uint256 timestamp,uint32 decimals) {
		try IPyth(pythAddress).getPriceUnsafe(id) returns (PythStructs.Price memory pyPrice
		) {
			price = uint256(uint64(pyPrice.price));
			timestamp = pyPrice.publishTime;
			if(pyPrice.expo > 0 ){
				decimals = uint32(pyPrice.expo);
			}else{
				decimals = uint32(-pyPrice.expo);
			}
		} catch {
